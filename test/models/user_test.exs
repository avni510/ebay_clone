defmodule EbayClone.UserTest do
  ...
  describe "changeset" do
    ...
    test "changeset is invalid if email does not exist" do
      attrs = Map.delete(@valid_attrs, :email)

      assert {:email, "can't be blank"} in errors_on(%User{}, attrs)
    end

    test "changeset is invalid if password does not exist" do
      attrs = Map.delete(@valid_attrs, :password)

      assert {:password, "can't be blank"} in errors_on(%User{}, attrs)
    end
  end
end
